...
import { Alert } from 'antd'; // Sử dụng Alert của Ant Design cho đồng bộ UI
...
export default function Login() {
    ...
    // Lấy thông điệp ẩn được gửi từ ProtectedRoute (nếu có)
    ...
    const navigateAfterAuth = (authResponse) => {
        ...
        // Nếu là Customer, kiểm tra xem trước đó họ có đang đặt dở vé không, nếu có thì trả về đúng trang đó, không thì về Home
        const fromPath = location.state?.from?.pathname || '/customer/home';
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-slate-50/60 p-5 sm:p-10 select-none">
            <div className="w-full max-w-md bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-xl shadow-slate-100">
                
                {/* Khối thương hiệu + Logo tràn viền đầy đặn */}
                <div className="mb-6 flex items-center justify-center gap-2.5">
                    <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white overflow-hidden shadow-sm shrink-0 border border-slate-200">
                        <img src="/logo.png" alt="SmartEvent Logo" className="h-full w-full object-cover" />
                    </div>
                    <span className="text-lg font-black text-slate-800 tracking-tight">SmartEvent</span>
                </div>
                ...
                <div className="mt-6">
                    <div className="relative flex justify-center text-sm">
                        ...
                        <span className="relative bg-white px-3 text-gray-500">hoặc đăng nhập với</span>
                    </div>
