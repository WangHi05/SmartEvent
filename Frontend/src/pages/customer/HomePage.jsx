...
import { ArrowRight, CalendarDays, Coffee, GalleryHorizontal, Laptop2, Mic2, Music2, Sparkles, Ticket, Users, Volleyball, Flame, ShieldCheck } from 'lucide-react';
...
const heroSlides = [
  {
    ...
    title: 'Khám phá sự kiện hot nhất theo phong cách Ticketbox',
    description: 'Hero banner lớn, card đẹp, thanh toán mượt và trải nghiệm mua vé chuyên nghiệp trên mọi thiết bị.',
    cta: 'Đặt vé ngay',
    accent: 'from-orange-500 via-amber-500 to-fuchsia-500',
  },
  {
    eyebrow: 'Sự kiện nổi bật',
    title: 'Concert, workshop, hội thảo và thể thao trong một hành trình duy nhất',
    description: 'Bộ lọc danh mục, ranking bán chạy, thông tin sức chứa và nút đặt vé được giữ nguyên logic.',
    cta: 'Khám phá sự kiện',
    accent: 'from-slate-900 via-slate-800 to-orange-600',
  },
  {
    eyebrow: 'SmartEvent for customers',
    title: 'Thiết kế hiện đại, responsive và thật như website bán vé chuyên nghiệp',
...
const HomePage = () => {
  ...
  const metrics = useMemo(() => {
    ...
    return [
      { label: 'Sự kiện đang mở bán', value: totalEvents.toLocaleString('vi-VN'), hint: 'Dữ liệu realtime từ hệ thống', icon: Ticket, accent: 'from-orange-500 to-amber-500' },
      { label: 'Đang diễn ra', value: liveEvents.toLocaleString('vi-VN'), hint: 'Theo dõi theo thời gian thực', icon: Flame, accent: 'from-emerald-500 to-teal-500' },
      { label: 'Sold-out', value: soldOutEvents.toLocaleString('vi-VN'), hint: 'Sự kiện đã hết chỗ', icon: Users, accent: 'from-slate-800 to-slate-600' },
    ];
  }, [events]);
  ...
  return (
    <div className="space-y-12 pb-4">
                  ...
                  <div className="max-w-3xl space-y-7">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <Button
                        type="primary"
                        size="large"
                        className="!h-12 !rounded-2xl !border-orange-400 !bg-white !px-6 !font-semibold !text-slate-950 shadow-xl"
                    ...
                    <div className="customer-glass relative overflow-hidden rounded-[34px] bg-white/12 p-5 text-white shadow-[0_30px_80px_rgba(15,23,42,0.25)] backdrop-blur-xl">
                      <div className="mb-4 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-lg font-bold">Bản xem nhanh sự kiện</p>
                        </div>
                        <div className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold">Live updates</div>
                      </div>
                      <div className="space-y-4">
                        <div className="rounded-[26px] bg-white/10 p-4">
                          <div className="mb-3 flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold">SmartEvent</p>
                              <p className="text-sm text-white/70">Premium customer journey</p>
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-3 text-sm text-white/80">
                            <div className="rounded-2xl bg-white/10 p-3">Carousel hero</div>
                            <div className="rounded-2xl bg-white/10 p-3">Event ranking</div>
                            <div className="rounded-2xl bg-white/10 p-3">Category filter</div>
                            <div className="rounded-2xl bg-white/10 p-3">Responsive grid</div>
                          </div>
                        </div>
                        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                          {categoryChips.slice(0, 3).map((category) => (
                            <button
                              ...
                              className="rounded-2xl border border-white/10 bg-white/8 px-4 py-3 text-left text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-white/14"
                            >
                              {category.label}
                            </button>
                          ))}
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <CustomerMetricCard icon={CalendarDays} label="Nội dung cập nhật" value={events.length.toLocaleString('vi-VN')} hint="Sự kiện đồng bộ từ hệ thống" accent="from-orange-500 to-amber-500" />
        <CustomerMetricCard icon={ShieldCheck} label="Thanh toán an toàn" value="VNPay" hint="Luồng thanh toán hiện có được giữ nguyên" accent="from-slate-800 to-slate-600" />
        <CustomerMetricCard icon={Music2} label="Nhiều chủ đề" value={EVENT_CATEGORIES.length - 2} hint="Nhạc sống, workshop, triển lãm..." accent="from-emerald-500 to-teal-500" />
      </section>
      ...
      <section className="space-y-5">
        <CustomerSectionTitle
          ...
          action={(
            <Link to="/customer/events" className="inline-flex items-center gap-2 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-orange-300 hover:text-orange-700">
              Xem toàn bộ sự kiện
              <ArrowRight size={16} />
            </Link>
          )}
      ...
      <section className="overflow-hidden rounded-[36px] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-orange-600 p-8 text-white shadow-[0_30px_80px_rgba(15,23,42,0.22)] lg:p-10">
        <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-center">
          <div className="space-y-4">
            ...
            <h3 className="text-3xl font-black tracking-tight sm:text-4xl">Mọi thứ sẵn sàng cho trải nghiệm đặt vé chuyên nghiệp</h3>
          ...
          <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
            <Button type="primary" size="large" className="!h-12 !rounded-2xl !border-white !bg-white !px-6 !font-semibold !text-slate-950" onClick={() => navigate('/customer/events')}>
              Bắt đầu khám phá
            </Button>
            <Button size="large" className="!h-12 !rounded-2xl !border-white/20 !bg-white/10 !px-6 !font-semibold !text-white hover:!border-white/35 hover:!bg-white/15" onClick={() => navigate('/customer/contact')}>
              Liên hệ hỗ trợ
            </Button>
          </div>
        </div>
